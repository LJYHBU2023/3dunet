...
def val(model, val_loader, loss_func, n_labels):
    ...
    with torch.no_grad():
        ...
        for idx, (data, target) in tqdm(enumerate(val_loader), total=len(val_loader)):
            ...
            # Debugging: Check shapes of output and target

            print(f"Val Batch {idx}: data shape: {data.shape}, target shape: {target.shape}, output shape: {output.shape}")
...
def train(model, train_loader, optimizer, loss_func, n_labels, alpha, epoch):
    ...
    for idx, (data, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        print(f"Train Batch {idx}: data shape: {data.shape}, target shape: {target.shape}, output shape: {output.shape}")
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_log = train(model, train_loader, optimizer, loss, args.n_labels, alpha, epoch)
        ...
        print(f'Best performance at Epoch: {best[0]} | {best[1]}')
