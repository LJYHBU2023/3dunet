...
class BoundaryLoss3D(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        elif len(targets.shape) == 5 and targets.shape[1] == 1:  # 假设targets的形状是(B, 1, D, H, W)
            targets = targets.squeeze(1)  # 移除单一通道维度

        targets_onehot = torch.nn.functional.one_hot(targets.long(), num_classes=logits.shape[1]).permute(0, 4, 1, 2, 3).float()

        dist_maps = []
        for i in range(logits.shape[1]):
            dist_map = distance((1 - targets_onehot[:, i].numpy()).astype(np.bool))
...
def val(model, val_loader, loss_func, n_labels, device):
    ...
    return val_log  # 确保 return 语句在函数内部
...
def train(model, train_loader, optimizer, loss_func, n_labels, device):
    ...
    return train_log  # 确保 return 语句在函数内部
