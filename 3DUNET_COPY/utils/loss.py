...
class BoundaryLoss(nn.Module):
    ...
    def forward(self, prob, dist_map):
        # prob: 输出概率, shape (B, C, D, H, W)
        # dist_map: 边界图, shape (B, 1, D, H, W)
        ...
        loss = 0
        for id in self.idc:
            class_logit = logit[:, id, :, :, :]
            class_dist_map = dist_map * (class_logit > 0).float()  # 仅考虑正样本
            loss += (class_dist_map ** 2).mean()
