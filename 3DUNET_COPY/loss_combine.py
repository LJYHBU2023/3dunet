...
class CombinedLoss(torch.nn.Module):
    ...
    def forward(self, probs, target):
        ...
        combined_loss = self.alpha * surface_loss + (1 - self.alpha) * tversky_loss
...
def train(model, train_loader, optimizer, loss_func, n_labels, device, epoch):
    ...
    for idx, (data, target) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        if isinstance(output, tuple):  # 检查输出是否为元组
            output = output[0]  # 取第一个元素作为输出
        output = output.softmax(dim=1)  # 确保输出是概率分布
        loss = loss_func(output, target)  # 使用 CombinedLoss
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    loss_func = CombinedLoss()  # 使用新的 CombinedLoss
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_log = train(model, train_loader, optimizer, loss_func, args.n_labels, device, epoch)
